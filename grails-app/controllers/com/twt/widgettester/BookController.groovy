...
class BookController {
	...
	def listJSON(Integer max) {
		params.max = Math.min(max ?: 10, 100)
		def jsonData = [jsonData: Book.list(params)] as JSON
		render jsonData
	}

	def fetchBooksByPublisherIdAsJSON(Integer max) {
		params.max = Math.min(max ?: 10, 100)
		def jsonData = [jsonData: Publisher.get(params.id)?.books] as JSON
		render jsonData
	}
